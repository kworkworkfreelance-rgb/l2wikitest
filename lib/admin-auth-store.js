...
const readExplicitEnvAuth = (fallbackUsername, fallbackPassword) => {
    const hasExplicitUsername = typeof process.env.ADMIN_USERNAME === 'string' && process.env.ADMIN_USERNAME.trim() !== '';
    const hasExplicitPassword = typeof process.env.ADMIN_PASSWORD === 'string' && process.env.ADMIN_PASSWORD.trim() !== '';

    if (!hasExplicitPassword) {
        return null;
    }

    const username = normalizeUsername(hasExplicitUsername ? process.env.ADMIN_USERNAME : fallbackUsername, fallbackUsername);
    const password = String(hasExplicitPassword ? process.env.ADMIN_PASSWORD : fallbackPassword || 'admin');
...
const normalizeAuthState = (payload, fallbackUsername, fallbackPassword) => {
    ...
    const passwordSource = [PASSWORD_SOURCE_BOOTSTRAP, PASSWORD_SOURCE_ENV, PASSWORD_SOURCE_MANUAL, PASSWORD_SOURCE_LEGACY].includes(
...
const ensureAdminAuthFile = (fallbackUsername, fallbackPassword) => {
    ...
    const envAuth = readExplicitEnvAuth(fallbackUsername, fallbackPassword);

    if (envAuth) {
        if (!fs.existsSync(AUTH_FILE_PATH)) {
            return writeAuthState(envAuth);
        }
        ...
        try {
            ...
            if (
                payload?.username !== envAuth.username ||
                payload?.passwordSource !== PASSWORD_SOURCE_ENV ||
                !verifyPassword(expectedPassword, payload?.passwordHash || '')
            ) {
                return writeAuthState(envAuth);
            }
...
const updateAdminPassword = (nextPassword, fallbackUsername, fallbackPassword) => {
    if (readExplicitEnvAuth(fallbackUsername, fallbackPassword)) {
        const error = new Error('Пароль администратора управляется ADMIN_PASSWORD из окружения.');
        error.status = 400;
        throw error;
    }
...
const isAdminAuthManagedByEnv = (fallbackUsername, fallbackPassword) => Boolean(readExplicitEnvAuth(fallbackUsername, fallbackPassword));
